...
class RenjuBoard(object):
    ...
    def Find_win(self):
        ...
        for sensitive in self.sensitive:
            coord = RenjuBoard.num2coordinate(sensitive)
            if self.isFive(coord,attacker):
                return sensitive,None,0
            if self.isFive(coord,defender):
                oppo_win = sensitive
                oppo_win_count += 1
    ...
    def _manage_sensitive(self,number):
        collects = []
        coord = RenjuBoard.num2coordinate(number)
        try:
            if self._(coord) == RenjuBoard.EMPTY_STONE:
                collects.append(coord.copy())
            else:
                self.sensitive.remove(number)
        except:
            pass

        for direction in RenjuBoard.direction8:
            _c = 0
            self._move_to(coord)
            stone = self.moveDirection(direction)
            while stone != False and _c < 2:
                if stone == RenjuBoard.EMPTY_STONE:
                    collects.append(self.current.copy())
                    _c += 1
                stone = self.moveDirection(direction)
        #这里我们收集了辐射8方向最多16个点
        #我们定义棋盘上的敏感位置： 当一个位置黑/白 落子形成四或者五 则为敏感位置。 
        for coordinate in collects:
            num = RenjuBoard.coordinate2number(coordinate)
            try:
                self.sensitive.remove(num)
            except:
                pass
            if (self.isFive(coordinate,RenjuBoard.BLACK_STONE) or self.isFive(coordinate,RenjuBoard.WHITE_STONE)) or (self.isFour(coordinate,RenjuBoard.BLACK_STONE) or self.isFour(coordinate,RenjuBoard.WHITE_STONE)):
                self.sensitive.append(num)
        #后面的全盘遍历尽量避免，只检查敏感位置。
